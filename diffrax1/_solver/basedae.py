...
from .._term import AbstractTerm, MultiTerm, AbstractTermDAE
...
def _term_compatible_contr_kwargs(term_structure):
    ...
    if isinstance(term_structure, type) and issubclass(term_structure, AbstractTermDAE):
        return {}
...
class AbstractSolver(eqx.Module, Generic[_SolverState], **_set_metaclass):
    ...
    term_structure: AbstractClassVar[PyTree[Type[AbstractTermDAE]]]
    ...
    def order(self, terms: PyTree[AbstractTermDAE]) -> Optional[int]:
        """Order of the solver for solving ODEs."""
        return None

    def strong_order(self, terms: PyTree[AbstractTermDAE]) -> Optional[RealScalarLike]:
        """Strong order of the solver for solving SDEs."""
        return None

    def error_order(self, terms: PyTree[AbstractTermDAE]) -> Optional[RealScalarLike]:
...
class HalfSolver(
    ...
    def func(
        self, terms: PyTree[AbstractTermDAE], t0: RealScalarLike, y0: Y, args: Args
    ) -> VF:
