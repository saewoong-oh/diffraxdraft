        ...
        if get_origin_no_specials(term_cls, error_msg) is MultiTerm:
            if isinstance(term, MultiTerm):
                ...
                for term, arg, term_contr_kwarg in zip(
                    ...
                    if not _term_compatible(yi, zi, args, term, arg, term_contr_kwarg):
                        raise ValueError
            else:
                raise ValueError
        else:
...
def daesolve(
    ...
    **Raises:**
    ...
    if isinstance(
        ...
        y0, z0, args, terms, (ODETerm, AbstractTerm), solver.term_compatible_contr_kwargs
    ):
        warnings.warn(
            "Passing `terms=(ODETerm(...), SomeOtherTerm(...))` to "
            f"{solver.__class__.__name__} is deprecated in favour of "
    ...
    def _allocate_output(subsaveat: SubSaveAt) -> SaveState:
        ...
        zs = jtu.tree_map(lambda z: jnp.full((out_size,) + z.shape, jnp.inf, dtype=z.dtype), struct)
        return SaveState(
