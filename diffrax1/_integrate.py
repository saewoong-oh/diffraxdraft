...
class SaveStateDAE(eqx.Module):
    ...
    zs: PyTree[eqxi.MaybeBuffer[Inexact[Array, "times ..."]]]
...
def loopdae(
    *,
    ...
    def body_fun_aux(state):
        ...
        # step sizes, all that jazz.
        #
        (y, y_error, z, z_error, dense_info, solver_state, solver_result) = solver.step(
            terms,
            state.tprev,
            state.tnext,
            state.y,
...
def _term_compatible_dae(
    ...
    error_msg = "term_structure"
    def _check(term_cls, term, term_contr_kwargs, yi, zi):
        if get_origin_no_specials(term_cls, error_msg) is MultiTerm:
            if isinstance(term, MultiTerm):
                [_tmp] = get_args(term_cls)
                assert get_origin(_tmp) in (tuple, Tuple), "Malformed term_structure"
                ...
                for term, arg, term_contr_kwarg in zip(
                    ...
                    if not _term_compatible_dae(yi, zi, args, term, arg, term_contr_kwarg):
                        raise ValueError
            else:
                raise ValueError
        else:
            ...
            # AbstractTerm[VF, Control] | (VF, Control)
            # -----------------------------------------
            term_args = get_args_of(AbstractTermDAE, term_cls, error_msg)
            n_term_args = len(term_args)
            if n_term_args == 0:
                pass
            elif n_term_args == 2:
                vf_type_expected, control_type_expected = term_args
                vf_type = eqx.filter_eval_shape(term.vf, 0.0, yi, zi, args)
                vf_type_compatible = eqx.filter_eval_shape(
                    better_isinstance, vf_type, vf_type_expected
                )
                if not vf_type_compatible:
    ...
    try:
        with jax.numpy_dtype_promotion("standard"):
            jtu.tree_map(_check, term_structure, terms, contr_kwargs, y, z)
...
def daesolve(
    ...
    **Raises:**
    ...
    y0 = jtu.tree_map(_promote, y0)
    z0 = jtu.tree_map(_promote, z0)
    del timelikes

    # Backward compatibility
    if isinstance(
        solver, (EulerHeun, ItoMilstein, StratonovichMilstein)
    ) and _term_compatible_dae(
        y0, z0, args, terms, (DAETerm, AbstractTermDAE), solver.term_compatible_contr_kwargs
    ):
        warnings.warn(
            "Passing `terms=(ODETerm(...), SomeOtherTerm(...))` to "
            f"{solver.__class__.__name__} is deprecated in favour of "
    ...
    # Error checking
    # if not _term_compatible_dae(
    #     y0, z0, args, terms, solver.term_structure, solver.term_compatible_contr_kwargs
    # ):
    #     raise ValueError(
    #         "`terms` must be a PyTree of `AbstractTerms` (such as `ODETerm`), with "
    #         f"structure {solver.term_structure}"
    ...
    def _wrap(term):
        assert isinstance(term, DAETerm)
        assert not isinstance(term, MultiTerm)
        return WrapTermDAE(term, direction)
    ...
    if controller_state is None:
        passed_controller_state = False
        (tnext, controller_state) = stepsize_controller.init(
            terms, t0, t1, y0, z0, dt0, args, solver.func, error_order
        )
    else:
        passed_controller_state = True
        if dt0 is None:
    ...
    if solver_state is None:
        passed_solver_state = False
        solver_state = solver.init(terms, t0, tnext, y0, z0, args)
    else:
        passed_solver_state = True
    ...
    if saveat.dense or event is not None:
        _, _, dense_info_struct, _, _ = eqx.filter_eval_shape(
            solver.step, terms, tprev, tnext, y0, z0, args, solver_state, made_jump
        )
    if saveat.dense:
        if max_steps is None:
            raise ValueError(
    ...
    ys = jtu.tree_map(lambda s: s.ys, final_state.save_state, is_leaf=is_save_state)
    zs = jtu.tree_map(lambda s: s.zs, final_state.save_state, is_leaf=is_save_state)
